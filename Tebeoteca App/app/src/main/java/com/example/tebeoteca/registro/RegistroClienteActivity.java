...
public class RegistroClienteActivity extends ComponentActivity {
    private EditText etNombreUsuario, etEmail, etContrasena, etNombreEmpresa, etFechaCreacionEmpresa;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        etFechaCreacionEmpresa = findViewById(R.id.et_FechaCreacionEmpresa);

        btnRegistroCliente = findViewById(R.id.btn_Registro_Cliente1);
    ...
    private void registroCliente() {
        ...
        String fechaCreacionEmpresa = etFechaCreacionEmpresa.getText().toString().trim();

        int selectedId = rgTipo.getCheckedRadioButtonId();
        RadioButton selectedOption = findViewById(selectedId);
        String tipo = selectedOption.getText().toString().trim().toLowerCase();

        if (nombreUsuario.isEmpty() || email.isEmpty() || contrasena.isEmpty() || nombreEmpresa.isEmpty() || fechaCreacionEmpresa.isEmpty() || tipo.isEmpty()) {
            Toast.makeText(this, "Completa todos los campos", Toast.LENGTH_SHORT).show();
            return;
        }

        RegistroClienteRequest registroClienteRequest = new RegistroClienteRequest(nombreUsuario,email,contrasena,nombreEmpresa,fechaCreacionEmpresa,tipo);
